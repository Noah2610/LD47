...
pub fn load_object(
    ...
    match &object.object_type {
        ObjectType::Player => {
            load_player::load_player(world, object, level_size)?;
        }

        ObjectType::Custom(object_ident) => {
            let object_settings = world
                .read_resource::<ObjectsSettings>()
                .objects
                .get(object_ident)
                .expect(&format!("No settings for object: {}", object_ident))
                .clone();
...
pub(super) fn load_objects(
    ...
    for object in objects {
        load_object(world, object, level_size.clone())?;
    }
